...
import torch
import utils_video
from transformers import AutoModel, AutoTokenizer, AutoConfig, BitsAndBytesConfig, LlamaForCausalLM, LlamaTokenizer
...
def get_embeddings_with_existing_hooks_forward(model, tokenizer, list_pixel_values, list_text_prompt, layer_outputs, counter, skip_pix=False):
    ...
    # for layer_name, embedding in layer_outputs.items():
    #     if layer_name not in all_embeddings:
    #         all_embeddings[layer_name] = []
    #     all_embeddings[layer_name].append(embedding.detach().cpu())
    
    # layer_outputs.clear()

    # # Concatenate batches
    # final_embeddings = {}
    # for layer_name, embedding_list in all_embeddings.items():
    #     final_embeddings[layer_name] = torch.cat(embedding_list, dim=0)
...
def get_embeddings_with_existing_hooks(model, tokenizer, list_pixel_values, list_text_prompt, layer_outputs, counter):
    ...
    return dict(layer_outputs), prompt_markers_list
def load_llama_model(model_path, use_quantization=True):
    """
...
# As an exemple, extract visual features for season 1, episode 1 of Friends
    ...
    if utils.isMockMode():
        ...
        model, tokenizer = load_llama_model(hf_path, use_quantization=True)
