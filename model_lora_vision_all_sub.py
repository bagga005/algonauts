...
class VisionLinearRegressionModel(nn.Module):
    def __init__(self, input_size, output_size, device, dropout_rate=0.2):
        ...
        self.linear42 = nn.Linear(input_size, output_size)
        self.linear43 = nn.Linear(input_size, output_size)
        self.linear44 = nn.Linear(input_size, output_size)
        nn.init.kaiming_normal_(self.linear41.weight)
        nn.init.kaiming_normal_(self.linear42.weight)
        nn.init.kaiming_normal_(self.linear43.weight)
        nn.init.kaiming_normal_(self.linear44.weight)
...
def train_on_device(rank, world_size, model_params, lora_p, lin_p, train_data, val_data, config):
    ...
    try:
        ...
        linear41_optimizer = torch.optim.Adam(
            model.module.linear41.parameters(), 
            lr=linear_learning_rate_initial, 
            weight_decay=linear_weight_decay
        )
        linear42_optimizer = torch.optim.Adam(
            model.module.linear42.parameters(), 
            lr=linear_learning_rate_initial, 
            weight_decay=linear_weight_decay
        )
        linear43_optimizer = torch.optim.Adam(
            model.module.linear43.parameters(), 
            lr=linear_learning_rate_initial, 
            weight_decay=linear_weight_decay
        )
        linear44_optimizer = torch.optim.Adam(
            model.module.linear44.parameters(), 
            lr=linear_learning_rate_initial, 
            weight_decay=linear_weight_decay
        )
        
        linear41_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            linear41_optimizer, 
            T_max=20,
            eta_min=linear_learning_rate_final
        )
        linear42_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            linear42_optimizer, 
            T_max=20,
            eta_min=linear_learning_rate_final
        )

        linear43_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            linear43_optimizer, 
            T_max=20,
            eta_min=linear_learning_rate_final
        )
        linear44_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            linear44_optimizer, 
            T_max=20,
            eta_min=linear_learning_rate_final
        )
        ...
        if rank == 0 and lin_optimizer_state_dict is not None and lin_scheduler_state_dict is not None:
            ...
            linear41_optimizer.load_state_dict(lin_optimizer_state_dict)
            linear41_scheduler.load_state_dict(lin_scheduler_state_dict)
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch_X, batch_y in train_loader:
                ...
                linear41_optimizer.zero_grad()
                linear42_optimizer.zero_grad()
                linear43_optimizer.zero_grad()
                linear44_optimizer.zero_grad()
                ...
                # Retain graph for all but the last backward pass
                if not train_only_linear:
                    # Use average loss for LoRA parameters
                    avg_loss.backward(retain_graph=True)
                    lora_optimizer.step()
                
                # Optimize each linear layer with its corresponding loss
                loss1.backward(retain_graph=True)
                linear41_optimizer.step()
                
                loss2.backward(retain_graph=True)
                linear42_optimizer.step()
                
                loss3.backward(retain_graph=True)
                linear43_optimizer.step()
                
                # No need to retain graph for the last backward pass
                loss4.backward()
                linear44_optimizer.step()
                ...
                if in_batch % 100 == 0 or in_batch < 10:
                    if rank == 0:  # Only print from main process
                        print(f'GPU {rank} | Epoch {epoch} | Batch {in_batch} | Loss(last batch): {avg_loss.item():.4f} | Loss (Avg in epoch): {total_loss/in_batch:.4f} | Loss1: {total_loss1/in_batch:.4f} | Loss2: {total_loss2/in_batch:.4f} | Loss3: {total_loss3/in_batch:.4f} | Loss4: {total_loss4/in_batch:.4f}')
            ...
            with torch.no_grad():
                for batch_X, batch_y in val_loader:
                    ...
                    val_loss42 += loss2.item()
                    val_loss43 += loss3.item()
                    val_loss44 += loss4.item()
            
            avg_val_loss = val_loss / len(val_loader)
            avg_val_loss41 = val_loss41 / len(val_loader)
            avg_val_loss42 = val_loss42 / len(val_loader)
            avg_val_loss43 = val_loss43 / len(val_loader)
            avg_val_loss44 = val_loss44 / len(val_loader)
            val_loss_tensor = torch.tensor(avg_val_loss).to(device)
            val_loss41_tensor = torch.tensor(avg_val_loss41).to(device)
            val_loss42_tensor = torch.tensor(avg_val_loss42).to(device)
            val_loss43_tensor = torch.tensor(avg_val_loss43).to(device)
            val_loss44_tensor = torch.tensor(avg_val_loss44).to(device)
            dist.all_reduce(val_loss_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_loss41_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_loss42_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_loss43_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_loss44_tensor, op=dist.ReduceOp.SUM)
            val_loss = val_loss_tensor.item() / world_size
            val_loss41 = val_loss41_tensor.item() / world_size
            val_loss42 = val_loss42_tensor.item() / world_size
            val_loss43 = val_loss43_tensor.item() / world_size
            val_loss44 = val_loss44_tensor.item() / world_size
            # Print progress on main process
            if rank == 0 and epoch % 1 == 0:
                print(f'Epoch {epoch:3d} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val Loss41: {val_loss41:.4f} | Val Loss42: {val_loss42:.4f} | Val Loss43: {val_loss43:.4f} | Val Loss44: {val_loss44:.4f}')

            # Step both schedulers at the end of each epoch
            lora_scheduler.step()
            linear41_scheduler.step()
            linear42_scheduler.step()
            linear43_scheduler.step()
            linear44_scheduler.step()
            ...
            if rank == 0:
                checkpoint = {
                    ...
                    'linear_optimizer': linear41_optimizer.state_dict(),
                    'lora_scheduler': lora_scheduler.state_dict(),
                    'linear_scheduler': linear41_scheduler.state_dict(),
            ...
            if rank == 0 and enable_wandb:
                logs = {
                    ...
                    "train/lr_linear": linear41_optimizer.param_groups[0]['lr'],
                    'test/loss': val_loss,
                    'test/loss41': val_loss41,
                    'test/loss42': val_loss42,
                    'test/loss43': val_loss43,
                    'test/loss44': val_loss44,
